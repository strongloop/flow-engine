/**
 * JsonValidate is a javascript based task. It is used to retrieve data over HTTP.
 **/
"use strict";
            ...
            input.readAsBuffer(function(error, buffer){
                ...
                try {
                    ...
                    if (result.errors.length > 0) {
                        ...
                        output.setVar("validation_error_detail", JSON.stringify(result.errors));
                    }
                    else
                    ...
                    output.setVar("validation_result", false);
                    output.setVar("validation_error_detail", JSON.stringify(errors));
                    resolve(output);
                }
            });
        });
...
//{ "id": "001", "name": "foo", "type": "jsonvalidate", "jsv_url": "http://9.191.74.66/firmware/type_address.jsv" }
function createTask(config) {
    if (util.isString(config.type) && config.type === "jsonvalidate")
        if (_isValidString(config.name))
            if (_isValidString(config.jsv_url))
                return new JsonValidate(config.name, config.jsv_url);
