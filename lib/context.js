...
/**
 * Context creates a context object with the given namespace. It will create
 * the namespace node and attaches to the context itself immediately.
 * like: 
...
function Context(namespace) {
    ...
    assert(!namespace ||
            (util.isString(namespace) && namespace.length > 0),
...
function setter(ns, name, value, readOnly) {
    assert(util.isString(name), 'name shall be a string');
...
function define(ns, name, getter, setter, configurable) {
    assert(util.isString(name), 'name shall be a string');
    assert(util.isFunction(getter), 'getter shall be a Function');
    //optional
    if ( util.isUndefined(setter) ) {
        configurable = true;
    } else {
        if ( util.isFunction(setter) ) {
            configurable = util.isUndefined(configurable) ? true :
                !!configurable;
        } else if ( util.isBoolean(setter) ) {
            configurable = setter;
            setter = undefined;
        } else {
            new Error('usage: define( name, getter, [setter, configruable])');
        }
    }
    var ctxNames = name.split('.');
    /*jshint validthis: true */
