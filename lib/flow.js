...
When the flow begins, it is in the kInit state. If there isn't any policy in the
assembly, the flow moves to kAbort state directly. Otherwise, it moves to
...
handler to fix the problem. The error handler might either succeed or fail. If
it is a success, the remaining flow will be resumed and move to kDone in the
end. If it is a failure, the global error handler will be invoked.

If the global error handler completes without error, the flow moves to
kPrepareOutput and kDone. However, if there is error unrecoverable in the global
error handler, the flow moves to kAbort.
...
In addition to main flow and subflows, the error handlers are also a series of
policies to take care of errors. Both of the policy and flow can define an error
handler.

   - A policy defines its own error handler to fix the problem in this policy
and its subflow (if there is one). When errors happen, the policy error handler
is invoked. After the policy error handler finishes, the flow to which the
policy at fault belongs can resume.

   - A flow may define its own error handler to fix the problem in it. For
example, the main flow may have a global error handler, or a subflow may have
its own error handler. When a flow goes wrong, its error handler is invoked.
After the error handler finishes, the issue is considered as recovered and the
flow is considered as finished - the remaining policies will not be executed.
(A custom policy may invoke a subflow with its own error handler via the
invokeFlow call. As for the built-in policies, they usually invoke a subflow
without one, but users may choose to define a policy error handler.)

If a flow ends with kAbort, the error object will be passed to the flow's
middleware callback. So the next error middleware can examine the error object.

TODO: add a section about how the response is written in the flow

*** enum of the flow type:
- kMain: the main flow
- kSubflow: a subflow invoked by other policies
- kPolicyErrorHandler: an error handler that is defined to handle the errors
    inside a policy
- kFlowErrorHandler: an error handler that is defined to handle the errors
    inside a flow
- kGlobalHandler: the global error handler, invoked when the error in the main
    flow is unrecoverable.
*/
const kMain = 0,
      kSubflow = 1,
      kPolicyErrorHandler = 2,
      kFlowErrorHandler = 3,
      kGlobalHandler = 4;
 ...
 * @param flowType The flow type (kMain, kSubflow, ..., etc)
...
Flow.prototype.prepare = function ( ctx, next ) {
    ...
    /*
     * invoke()
     * a policy could use this to execute a sub-flow 
     */
    Object.defineProperty(flow, 'invoke',
            {   'value': function (assembly, next, options) {
                    invokeFlow.call(_this, assembly, next, options);
                },
                writable: false,
                enumerable: true,
                configurable: false});
    ...
    /*
     * subscribe()
     * a policy could subscribe a special event
     */
    Object.defineProperty(flow, 'subscribe',
    ...
    /*
     * unsubscribe()
     * a policy could unsubscribe a special event
     */
    Object.defineProperty(flow, 'unsubscribe',
            {   'value': function(event, listener) {
...
Flow.prototype.run = function () {
    var _this = this;
    this.logger.debug('Notify subscriber for the START event');

    //done callback for the notify()
    function startEventDone ( error ) {
        if ( error ) {
            _this.logger.error(
                    'Error in calling the subscribers of START event');
        }
    ...
    Observable.notify.call(
            _this, ['pre:START', 'START', 'post:START'],
            'START', //event name
...
Flow.prototype._evaluateState = function() {
    ...
    case kInit:
    {
        ...
        this.logger.info('Executing assembly with flow id "%d"', mainFlow.id);
    ...
    case kDone:
        ...
        this.logger.debug('Notify subscriber for the FINISH event');
        //treating the pre and post are the same. and ignore errors in 
        //event handlers
        Observable.notify.call(_this, ['pre:FINISH', 'FINISH', 'post:FINISH'],
                'FINISH', //event name
                function(error, next) { //intermittent callback
                    if (error) {
    ...
    case kAbort:
        ...
        //treating the pre and post are the same. and ignore errors in 
        //event handlers
        Observable.notify.call(_this, ['pre:ERROR', 'ERROR', 'post:ERROR'],
                'ERROR', //event name
                function(error, next) { //intermittent callback
                    if (error) {
...
Flow.prototype._prepareNextTask = function () {
    ...
    if ( task ) {
        ...
        var next = function(error) {
            taskNext.call(_this, task, error);
        };
        ...
        try {
            ...
            var preEventDone = function (error) {
                if (error) {
                    _this.logger.error('Error in pre:%s event:%s',
                            taskType, error);
                }
                //execute the task now
                taskHandler(_this._context, next);
            };
            _this.logger.debug('Notify subscriber for the pre:%s event',
                    taskType);
        ...
        if ( this._flowStack.length === 1 ) {
            ...
            //This flow is completed. Continue with remaining flows on the stack
            var finished = this._flowStack.pop();
            var nextf = this._flowStack[this._flowStack.length - 1];

            if ( finished.resume ) {
                if (finished.flowType === kSubflow) {
                    this.logger.info('The current subflow (id=%d) is done. ' +
                    ...
                    this.logger.info('The flow error handler (id=%d) is done.' +
                ...
                this.logger.info('The policy error handler has recovered the ' +
...
specify their error handlers, L0 and L2 to take care of errors. L2 can deal with
the specific error 'a', 'b', and 'c', while L2 can deal with any errors. In
addition, the main flow and the subflow S1 got their own error handlers, GEH and
L1 to fix any errors at the flow-level. (The * is used to indicate a match-all,
or the default case)
...
*** case 1: flow error handler
When an error 'z' is thrown from the second policy of S2, L2 can't handle the
error while the flow's error handler L1 can. So the subflows, S2 and S1, are
removed from the stack (or aborted) and L1 is pushed onto stack. L1 now is
invoked to handle the error 'a'.
...
What if another error is thrown from the L1? First, we need to check whether the
policy itself can handle the error or not. If not, pop L1 first, and then try
L0. If L0 cannot deal with the error either, pop the main and push the GEH.


*** case 2: policy error handler
When an error 'b' is thrown from S2, The policy error handler of C2, 'L2', is
invoked to handle errors. So S2 is popped and L2 is pushed.
...
What if the error 'b' is re-thrown from the L2_b? First, we need to check whether
the policy itself can handle the error or not. If not, pop L2_b. Then we check
L1 instead of L2 here. Why skip L2? Because the L2 has already invoked and
failed. So it is time check the L1. If L1 can handle the error, pop S1 and push
L1. Otherwise, check L0 and then GEH. If L2 is not skipped, it becomes an
infinite loop here.
...
function taskNext() {
    ...
    if ( error === undefined ) {
        ...
        var postEventDone = function (error) {
            if (error) {
                that.logger.error('Error in post:%s, error:%s',
                        taskType, error);
            }
            that._evaluateState();
        };
        var ev = 'post:' + taskType;
        that.logger.debug('Notify subscriber for the post:%s event',
                taskType);
        ...
        while (that._flowStack.length > 0) {
            ...
            if (currFlow.catch) {
                ...
                if (handler) {
                    ...
                    if (currFlow.flowType === kMain) {
                        ...
                        that.logger.info('The main flow aborted. Invoke the ' +
                        ...
                        that.logger.info('The subflow (id=%d) aborted. Invoke' +
            ...
            if (currFlow.flowType === kPolicyErrorHandler) {
                that.logger.info('The policy error handler (id=%d) cannot' +
                        ' deal with the error. Abort now.', currFlow.id);
            }
            else if (currFlow.flowType === kFlowErrorHandler) {
                that.logger.info('The flow error handler (id=%d) cannot' +
                        ' deal with the error. Abort now.', currFlow.id);
            }
            else {
 ...
 * handler, they can provide an assembly in a JSON object like this:
