...
Flow.prototype.prepare = function ( req, resp, next ) {
    ...
    this._resp = resp;
    this._next = next;
    this._context = req.ctx || createContext(req, this._options);
}
...
Flow.prototype._prepareNextTask = function () {
    ...
    if ( task ) {
        ...
        try {
            ...
            var taskModule;
            try {
               taskModule = this._options.tasks[taskType] || require('./task/' + taskType);
            } catch (err) {
    ...
    for (var i in catchClause) {
        ...
        if (errCase.errors) {
            for (var test in errCase.errors) {
                ...
                try {
                    //TODO: the thrown error is supposed to be an Object and will be used as the context.
                    //      what should we do with the error string 'invoke-error'?
                    //      The workaround is to put it into the context.error. It must be fixed.
                    if (!(error instanceof Object)) {
                        error = { "context": { "error": error } };
                    }

                    var script = new vm.Script(text);
                    var fakeCtx = new vm.createContext(error);
