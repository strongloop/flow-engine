...
Flow.prototype._prepareNextTask = function() {
  ...
  if (task) {
    ...
    // The policy implementation may be referenced in this.options.tasks
    // by taskType or taskType:version if version is specified.
    var taskIndex = (!task.version) ? taskType : taskType + ':' + task.version;
    ...
    try {
      ...
      var taskHandler = current.getTaskFunc(taskIndex) ||
...
function taskFail(task, error) {
  ...
  while (that._flowStack.length > 0) {
    ...
    // TODO: TCB - figure out why currTask[type] is null with the new  
    //       policy loader
    if (currTask && currTask[type] && currTask[type].catch) {
