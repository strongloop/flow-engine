 ...
 * @param taskParamResolve is a function for resolving task
 *        parameters
 */
...
Flow.prototype._resolveTaskParam = function(taskCfg) {
    ...
    let result = {};
    let keys = Object.keys(taskCfg);
    for (let i = 0; i<keys.length; i++) {
        let name = keys[i];
        let value = taskCfg[name];

        if (util.isObject(value)) {
            result[name] = this._resolveTaskParam(value);
        } else if (util.isArray(value)) {
            result[name] = [];
            for (let cursor = 0; cursor<value.length; cursor++) {
                result.push(this._resolveTaskParam(value[cursor]));
            }
        } else if (util.isPrimitive(value)) {
            // call user's custom parameter resolve function to resolve value
            result[name] = this._taskParamResolver(name, value);
        } else {
            throw new Error("unexpected data type for task param resolving");
        }
    }
