 ...
 * @param taskParamResolver is a function for resolving task
 *        parameters
 */
function Flow (config, taskParamResolver) {
    if ( !(this instanceof Flow) ) {
        return new Flow(config, taskParamResolver);
    }

    if ( !config ) {
        throw new SyntaxError("new Flow(config [, taskParamResolver])");
    }
    ...
    this._taskParamResolver = taskParamResolver;
...
Flow.prototype.run = function () {
    ...
    this._context.bind( function () {
        _this._context.set('logger', _this.logger);
...
Flow.prototype._resolveTaskParam = function(taskCfg) {
    if (util.isUndefined(this._taskParamResolver)) {
    ...
    for (var key in taskCfg) {
        if (taskCfg.hasOwnProperty(key)) {
            ...
            taskCfg[key] = util.isPrimitive(value) ?
                this._taskParamResolver(this._context, key, value) :
                _this._resolveTaskParam(value);
        }
    }
    this.logger.debug('After resolve: ' + JSON.stringify(taskCfg, undefined, 2));
    return taskCfg;
};
...
function taskNext(task, error) {
    if ( arguments[1] instanceof Error ) {
        ...
        if ( this._state == kRunningFlow ) {
            ...
            this.logger.error("Found an error in the global error handler: " + e);
            this._error = e;
            this._state = kAbort;
        }
        else if ( this._state == kRunningLocalError) {
            this.logger.error("Found an error in the local error handler: " + e);
