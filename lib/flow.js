...
const gLogger = bunyan.createLogger(
        {
            name:'flow-engine',
...
/**
 * load the built-in tasks here. these built-in tasks are the very
 * basic ones. however, some may be moved to micro-gw later
 */
const builtInTasks = {
        'if': require('./task/if')(),
...
Flow.prototype._evaluateState = function() {
    ...
    case kDone:
        ...
        //Notify the FINISH event first. Then continue with the next middleware.
 ...
 * @param flow the flow-engine instance itself
 * @param task the running task
 * @param taskType the running task's type, aka policy name
 */
function createFlowObj(flow, task, taskType) {
    ...
    rev.fail = function(error) {
        if (called) {
            flow.logger.warn(
                'call .fail() more then once in policy "%s"', taskType);
        } else {
    ...
    rev.proceed = function() {
        if (called) {
            flow.logger.warn(
                'call .proceed() more then once in policy "%s"', taskType);
        } else {
    ...
    rev.stop = function() {
        if (called) {
            flow.logger.warn(
                'call .stop() more then once in policy "%s"', taskType);
        } else {
