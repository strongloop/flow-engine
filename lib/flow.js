...
Flow.prototype._evaluateState = function() {
    ...
    case kAbort:
        ...
        if (!this._error)
            this.logger.warn('Internal error? The abort reason is undefined.');
...
Flow.prototype._prepareNextTask = function () {
    ...
    if ( task ) {
        this.logger.info ('loading task #' +
                current.id + '-' + current.taskIndex + ':', task);
        ...
        try {
            ...
            this.logger.error('Exception with loading task #' +
                    current.id + '-' + current.taskIndex + ':', e);
        ...
        if ( this._flowStack.length === 1 ) {
            this.logger.info('All tasks have been executed.');
...
function taskNext() {
    ...
    if ( error === undefined ) {
        ...
        current = that._flowStack[that._flowStack.length - 1];
        current.taskIndex++;
    }
    else {
        ...
        that.logger.error('The task "', Object.getOwnPropertyNames(caller)[0], '" returned with an error "', JSON.stringify(error), '"');

        var errorHandler;
        var resolvedCfg;
        if ( that._state === kRunningFlow ) {
            //try the local error handler
            do {
                current = that._flowStack.pop();
                var task = current.flow[current.taskIndex];
                var taskType = Object.getOwnPropertyNames(task)[0];
                var paramResolver = current.getParamResolver();
                var taskCfg = task[taskType];
                var errorCatch = taskCfg.catch || current.errorCatch;
                that.logger.debug('Looking the local error handler in task "', taskType, '"');

                if (errorCatch) {
                    //find task base local error handing first
                    resolvedCfg = that._resolveTaskParam(paramResolver, errorCatch);
                    errorHandler = that.findMatchingErrorHandler(resolvedCfg, error);
                    //no matched, check sub-flow level local error handing if there is
                    if ( !errorHandler && taskCfg.catch && current.errorCatch ) {
                        resolvedCfg = that._resolveTaskParam(paramResolver, current.errorCatch);
                        errorHandler = that.findMatchingErrorHandler(resolvedCfg, error);
                    }
                    if (errorHandler) {
                        that.logger.info('Invoke the local error handler from the activity"', taskType, '"');
                        that._state = kRunningLocalError;
                        that._flowStack.push(new FlowStackObj(that._flowID++, errorHandler, kErrorFlow, {}, current));
                        break;
                    }
                }
            } while (!errorHandler && that._flowStack.length > 0);

            //try the global error handler
            if (!errorHandler) {
                that.logger.info('Cannot find the local error handler.');

                if (that._globalErrorHandler) {
                    resolvedCfg = that._resolveTaskParam(that._options.paramResolver, that._globalErrorHandler);
                    errorHandler = that.findMatchingErrorHandler(resolvedCfg, error);
                }

                if (errorHandler) {
                    that.logger.info('Invoke the global error handler.');

                    //invoke the error handler
                    that._state = kRunningGlobalError;
                    that._flowStack.push(new FlowStackObj(that._flowID++, errorHandler, kErrorFlow, that._options));
                }
                else {
                    that.logger.info('Cannot find the global error handler either. Abort now');
        ...
        else if ( that._state === kRunningLocalError) {
            that.logger.error('Found an error in the local error handler: ', error);
            ...
            if (errorHandler) {
                that.logger.info('Turn to the global error handler.');

                //invoke the error handler
                that._state = kRunningGlobalError;
                that._flowStack.push(new FlowStackObj(
                            that._flowID++, errorHandler, kErrorFlow, that._options));
            }
            else {
                that.logger.info('Cannot find the global error handler. Abort now');

                that._error = error;
                that._state = kAbort;
            }
        }
        else if ( that._state === kRunningGlobalError ) {
            that.logger.error('Found an error in the global error handler: ', error);
            that._error = error;
            that._state = kAbort;
        }
        else {
...
function invokeFlow(assembly, next, options) {
    ...
    var current = that._flowStack[that._flowStack.length - 1];
    var task = current.flow[current.taskIndex];
    var taskType = Object.getOwnPropertyNames(task)[0];

    var newFlowID = that._flowID++;
    that.logger.info('A subflow (id=', newFlowID, ') is invoked by the "', taskType, '" task');
    ...
    that._flowStack.push(flowStackObj);
